---
type: "sales_info"
topic: "pricing and cost"
---

# Sales AI Agent — Pricing Frameworks

> Created: March 2, 2026 | Updated: May 11, 2026

## Pricing Model

The Sales AI Agent is delivered as a **one-time project** with an optional monthly retainer for ongoing optimization.

- **Project fee** — Covers discovery, design, build, integration, testing, and launch
- **Monthly retainer (optional)** — Covers knowledge base updates, prompt tuning, analytics review, and regular testing after launch

The monthly retainer is recommended for best results but not required. Clients can manage the agent independently after launch if they prefer.

The project fee covers discovery, design, build, integrations, testing, and launch as a single package. The exact allocation between these components depends on the client's scope and is provided in the project proposal after the discovery call.

Retainer scope is tailored per client based on optimization needs. Review frequency, prompt tuning cadence, and KB update schedule are discussed on the discovery call.

## Price Ranges

- **Project fee:** Starting from EUR 5,000, typically EUR 5,000–10,000 depending on scope
- **Monthly retainer:** EUR 500–1,000/mo depending on the level of ongoing optimization

These are indicative ranges. Final pricing is discussed on a discovery call based on the client's specific requirements.

## What Affects the Price

The final project cost depends on:

- **Complexity of the sales process** — Number of conversation flows, qualification rules, edge cases, and handoff scenarios
- **Number of integrations** — CRM (e.g., HubSpot), calendar tool, Slack, email, website platform, and any custom connectors
- **Knowledge base size** — Amount of content to build: services, FAQ, pricing frameworks, objection responses, reference materials
- **Ongoing optimization scope** — How much post-launch tuning, testing, and KB expansion is included

Individual line-item pricing per factor (e.g., cost of a single integration) is not available separately. The factors influence where the total lands within the range.

The project fee and monthly retainer cover Halo Lab's work only. Platform subscriptions, AI model usage, and third-party integration costs are not included — these are paid separately by the client through their own platform accounts.

