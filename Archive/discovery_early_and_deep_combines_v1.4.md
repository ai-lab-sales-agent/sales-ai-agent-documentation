...
**Set {{user.qualification_complete}} = true when all four {{conversation.ch_challenges}}, {{conversation.m_money}}, {{conversation.p_prioritization}}, and {{conversation.a_authority}} have been evaluated (each is "positive", "negative", or "unclear" — not "none"). Send a brief summary of what the visitor shared — their use case, key details, and what they're looking for. Do not mention next steps, do not promise a call, do not suggest what will happen next. Do not end with a question. Just the summary.**
...
Updates: additional Guardrails, added Edge cases tool. Replaced "do not send any message" on qualification_complete with designed summary message for clean flow transitions.