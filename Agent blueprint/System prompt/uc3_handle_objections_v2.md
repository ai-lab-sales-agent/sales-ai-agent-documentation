...
3.1. If the visitor confirms → set {{workflow.objection_addressed}} to 'true'. Wait for transition back to the calling node, do not write any messages. 
3.2. If the visitor pushes back/continues asking questions → address with different KB content, then repeat 3. **Check**
3.3. If you run out of new KB content to share → set {{workflow.objection_addressed}} to 'true'. Wait for transition back to the calling node, do not write any messages. 
...
Updates: changed structure, added Edge Cases tool, additional Guardrails, updated Handling objection pattern. 