...
> Created: February 24, 2026 | Updated: March 17, 2026
...
```json
{
  ...
  "title": "SalesAgentConversation_v2.1",
  "description": "Conversation variables for CHAMP Scored Flow. Agent evaluates CHAMP signals and routes leads automatically: Hot/Warm → Cal.com booking link (tagged), Nurture → resources + re-qualify, DQ → polite close. Budget < €5,000 = DQ.",
  "type": "object",
  "properties": {
    ...
    "visitor_email": {
      "type": ["string", "null"],
      "format": "email",
      "description": "Email address. Collected via contact form or Cal.com booking.",
      "default": null
    },
    ...
    "icp_exclusion_flag": {
      "type": "boolean",
      "description": "True if ICP hard exclusion detected at Step 3. Triggers AUTO DQ — immediate polite close, no booking link, no form.",
      "default": false
    },

    "ch_challenges": {
      "type": "string",
      "enum": ["positive", "negative", "unclear", "none"],
    ...
    "p_prioritization": {
      "type": "string",
      "enum": ["positive", "negative", "unclear", "none"],
      "description": "P — Prioritization. POSITIVE: realistic deadline or trigger event. NEGATIVE: 'someday' or no priority. MODERATE weight — differentiates Hot from Warm.",
      "default": "none"
    },
    ...
    "conversion_action": {
      "type": "string",
      "enum": ["booking_link_shared", "form_submitted", "resources_sent", "none"],
      "description": "What conversion action occurred. Bot shares Cal.com link but cannot confirm if visitor actually booked.",
      "default": "none"
    },

    "booking_fallback_used": {
      "type": "boolean",
      "description": "True if visitor declined the Cal.com booking link and the agent fell back to the contact form.",
      "default": false
    },

    "pre_call_brief_sent": {
      "type": "boolean",
      "description": "Not active for MVP. Bot cannot confirm Cal.com bookings, so there is no reliable trigger for the pre-call brief.",
      "default": false
    },
...
> CHAMP signal columns removed from Leads Table — signals stay in conversation variables only. See `botpress_build_summary.md` section 10.

```json
{
  "$schema": "http://json-schema.org/draft-07/schema#",
  "title": "LeadsTable_v2.1",
  "description": "Botpress Table schema for persisting lead data. One row per qualified conversation. Used by sales team to review leads. Budget < €5,000 = DQ.",
  "type": "object",
  "properties": {

    "visitor_id":           { "type": "string",  "description": "Auto-generated by Botpress from built-in userId. Persisted via browser cookie. Used as row identifier." },
    ...
    "conversion_action":    { "type": "string",  "enum": ["booking_link_shared", "form_submitted", "resources_sent", "none"] },
    "booking_fallback_used": { "type": "boolean", "default": false, "description": "True if visitor declined Cal.com link, agent fell back to contact form." },
    "pre_call_brief_sent":  { "type": "boolean", "default": false, "description": "Not active for MVP." },
    ...
    "resources_shared":     { "type": "array", "items": { "type": "string" }, "description": "List of case study / resource links shared with the visitor at Nurture N1.", "default": [] },
...
```json
{
  ...
  "description": "Contact form schema. Triggered when visitor declines Cal.com booking link. For knowledge gaps, the agent offers an alternative and provides a contact email instead of a form — see Knowledge Gap Escalation in Qualification framework v2.",
  "type": "object",
  "properties": {
...
|-------------|------------------|----------------|----------------|---------------------|-----------------------------------------|
| **Hot**     | confirmed        | confirmed      | confirmed      | confirmed           | Cal.com booking link (tag: Hot)         |
| **Warm**    | confirmed        | confirmed or missing | confirmed or missing | confirmed or missing | Cal.com booking link (tag: Warm) |
